...
class SubSetting {
    ...
    public function getPrettyKey($url = true) {
        $out = str_replace(helper_plugin_subconfhelper_config::KEYMARKER, "»", $this->key);
    ...
    public function getArrayKey() {
        return str_replace(helper_plugin_subconfhelper_config::KEYMARKER, "']['", $this->key);
    }
    ...
    public function getType() {
        if(substr($this->getKey(), 0, 10) == 'plugin' . helper_plugin_subconfhelper_config::KEYMARKER) {
            return 'plugin';
        } else if(substr($this->getKey(), 0, 7) == 'tpl' . helper_plugin_subconfhelper_config::KEYMARKER) {
            return 'template';
        } else {
            return 'dokuwiki';
        }
    ...
    public function caution() {
        ...
        if(!in_array($this->caution, SubSetting::$validCautions)) {
...
class SubSettingHidden extends SubSetting {
  // Used to explicitly ignore a setting in the configuration manager.
}


class SubSettingUndefined extends SubSettingHidden {
    ...
    public function html( $plugin, $echo = false) {
        ...
        if(preg_match(
            '/^(?:plugin|tpl)' . helper_plugin_subconfhelper_config::KEYMARKER . '.*?' . helper_plugin_subconfhelper_config::KEYMARKER . '(.*)$/',
            $this->getKey(),
...
class SubSettingNoClass extends SubSettingUndefined {
    protected $errorMessage = '_msg_setting_no_class';
}

class SubSettingString extends SubSetting {
    /** @inheritdoc */
    public function html( $plugin, $echo = false) {
        $disable = '';
